...
export default {
  ...
  computed: {
    s_sichosonmei: {
      get() {
        return this.$store.state.info.h07citysichosonmei[this.mapName]
      },
      set(value) {
        this.$store.state.info.h07citysichosonmei[this.mapName] = value
        LayersMvt.cityH07Obj[this.mapName].getSource().changed()
        this.storeUpdate()
      }
    },
    s_selectColor: {
      get() {
        return this.$store.state.info.h07citySelectColor[this.mapName]
      },
      set(value) {
        this.$store.state.info.h07citySelectColor[this.mapName] = value
        LayersMvt.cityH07Obj[this.mapName].getSource().changed()
        this.storeUpdate()
      }
    },
