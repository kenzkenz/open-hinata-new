...
export default {
  ...
  computed: {
    s_sichosonmei: {
      get() {
        return this.$store.state.info.r05citysichosonmei[this.mapName]
      },
      set(value) {
        this.$store.state.info.r05citysichosonmei[this.mapName] = value
        LayersMvt.cityR05Obj[this.mapName].getSource().changed()
        this.storeUpdate()
      }
    },
    s_selectColor: {
      get() {
        return this.$store.state.info.r05citySelectColor[this.mapName]
      },
      set(value) {
        this.$store.state.info.r05citySelectColor[this.mapName] = value
        LayersMvt.cityR05Obj[this.mapName].getSource().changed()
        this.storeUpdate()
      }
    },
